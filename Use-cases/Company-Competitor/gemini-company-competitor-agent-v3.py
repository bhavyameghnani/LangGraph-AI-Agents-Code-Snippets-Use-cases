...
def semantic_filter(state: CompetitorState):
    """
    Use HuggingFaceEmbeddings to select top 5 most semantically similar competitors
    ...
    comp_embs = [embeddings.embed_query(c) for c in competitors]

    # Compute cosine similarity
    scores = []
    for comp, emb in zip(competitors, comp_embs):
        sim = np.dot(query_emb, emb) / (np.linalg.norm(query_emb) * np.linalg.norm(emb))
        scores.append((comp, sim))
