...
Design update (two-stage agent):
1) Discovery Agent
   - Scans all CSVs in INPUT_DIR
   - Reads samples from every file and builds a combined view
   - Calls Gemini once (or per distinct schema group) to classify columns into PII/PHI/NONE and suggest subtypes
...
Notes & improvements over previous version:
- Correct use of faker instance seeding (faker.seed_instance)
- Link detection across columns to preserve referential integrity (account IDs, client IDs, emails)
- Single classification step so Gemini "thinks" about the whole dataset first
- Parallelized anonymization for speed

Usage
- Set INPUT_DIR and OUTPUT_DIR at top of file
- Ensure GOOGLE_API_KEY/Gemini access (via .env) and required packages installed
...
SAMPLE_ROWS = 20                 # how many sample rows to send to Gemini for classification
...
def build_combined_prompt(all_samples: Dict[str, pd.DataFrame]) -> str:
    """Build a single prompt for Gemini that contains samples from all files and asks for classification per column name.
...
def call_gemini_combined(all_samples: Dict[str, pd.DataFrame]) -> List[Dict[str, Any]]:
...
def build_global_plan(csv_paths: List[str]) -> Tuple[Dict[str, Any], List[List[Tuple[str, str]]]]:
    """Stage 1 (Discovery): read samples, call Gemini, detect links, return col_info and link_groups."""
    samples = {}
    full_dfs = {}
    for p in csv_paths:
    ...
    print("Calling Gemini to classify columns across all files...")
    classifications = call_gemini_combined(samples)
